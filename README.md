...
| **이미지 포맷 변경 (jpg -> webp)**      | 이미지 요청 용량 개선              | jpg -> webp                         | LCP 개선                  |
| **제품 이미지 Lazy Loading 적용**       | 초기 로딩 시간 단축                | img 요소에 `loading=lazy` 추가      | LCP 개선                  |
| 이미지 요소에 alt 값 추가               | SEO, 접근성 지표 향상              | img 요소에 `alt` 추가               | SEO, 접근성 지표 개선     |
| main.js 최적화                          | 동일 요소 중복 참조                | 참조 요소 변수로 캐싱               | TBT 개선                  |
| 폰트 최적화                             | 로딩 시간 단축                     | 폰트 자체 호스팅                    | LCP 개선                  |
| **이미지 포맷 2차 변경 (webp -> avif)** | 이미지 요청 용량 개선              | webp -> avif                        | LCP 개선                  |
| products.js 최적화                      | 무거운 연산 개선                   | 무거운 연산 개선                    | TBT 개선                  |
| **index.html head 내 script 최적화**    | 초기 로딩 속도 개선                | 스크립트 지연 로딩                  | LCP, TBT 개선             |
